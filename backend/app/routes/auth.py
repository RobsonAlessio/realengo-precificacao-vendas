...
def login(payload: schemas.LoginRequest, db: Session = Depends(get_db)):
    ...
    if ldap_auth:
        ...
        if not user:
            # Cria novo usuário LDAP
            user = models.User(
                username=payload.username,
                hashed_password=None,  # Usuários LDAP não têm senha local
                auth_provider="ldap",
                is_active=True,
                role=None,  # Pendente aprovação
            )
            db.add(user)
            db.commit()
            db.refresh(user)
        ...
        # Verifica se o usuário LDAP tem role
